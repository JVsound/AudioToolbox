clearvars; close all; clc;
%[text] # Class: result
%[text] JVsound, J.G. Vermond
disp(datetime("today")) %[output:3a6d6af6]
%[text:tableOfContents]{"heading":"**Table of Contents**"}
%[text] ## Description
%[text] 
%[text] ## Properties
%[text] ### Value
%[text] 
%[text] ### Dependent
%[text] #### `ElectricalImpedance`
%[text] %[text:anchor:TMP_93f5] 
%[text] #### `Pressure`
%[text] Pressure \[Pa\] at distance `ObservationRadius` (r in the equation below). The applied equation depend on the radiation angle. 
%[text] Radiation from a monopole point source (simple source) is as follows:
%[text] $p(r) = jk\\rho c Q \\frac{e^{-jkr}}{4\\pi r}$
%[text] Far-field on 
%[text] #### `SoundPressureLevel`
%[text] 

%[appendix]{"version":"1.0"}
%---
%[metadata:view]
%   data: {"layout":"inline","rightPanelPercent":40}
%---
%[output:3a6d6af6]
%   data: {"dataType":"text","outputData":{"text":"   04-Sep-2026\n\n","truncated":false}}
%---
